...
```shell
helm install postgresql bitnami/postgresql --values ./values/postgresql/values.minikube.yaml
```
...
```shell
kubectl apply -f ./resources/keycloak-custom-config-vars.local.yaml
kubectl apply -f ./resources/keycloak-custom-secret-vars.local.yaml
```
